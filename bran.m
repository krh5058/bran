...
if s
    ...
    for i = 1:length(obj.exp.order)
        ...
        while (GetSecs - t0) < obj.exp.presmatend(order_i) % Continue until last time point
            ...
            if prepflag
                ...
                data.schedt = obj.getT(iii,ii,order_i);
            ...
            if dispflag
                if (GetSecs - t0) > data.schedt % If time surpasses current onset
                    ...
                    try
                        ...
                        if obj.debug
                            disp(['bran.m (Debug): Scheduled time: ' num2str(data.schedt)]);
                            disp(['bran.m (Debug): VBL timestamp: ' num2str(data.actt)]);
